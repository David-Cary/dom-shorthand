 ...
 * @returns {boolean}
 */
function attributesMatchDescription(attributes, description) {
    var count = 0;
    for (var key in description) {
        var attribute = attributes.getNamedItem(key);
...
exports.attributesMatchDescription = attributesMatchDescription;
/**
 ...
 * @returns {boolean}
 */
function nodeMatchesDescription(node, description) {
    var _a, _b;
    if (node.nodeType !== description.nodeType)
        return false;
    if (description.nodeName != null &&
    ...
    if (node instanceof Element) {
        var describedAttributes = (_a = description.attributes) !== null && _a !== void 0 ? _a : {};
        var attributesMatch = attributesMatchDescription(node.attributes, describedAttributes);
        if (!attributesMatch)
            return false;
    }
    else if (description.attributes != null)
        return false;
    var describedChildren = (_b = description.childNodes) !== null && _b !== void 0 ? _b : [];
    var childrenMatch = nodeListMatchesDescription(node.childNodes, describedChildren);
    return childrenMatch;
}
exports.nodeMatchesDescription = nodeMatchesDescription;
/**
 ...
 * @returns {boolean}
 */
function nodeListMatchesDescription(nodes, descriptions) {
    for (var i = 0; i < descriptions.length; i++) {
        var node = nodes.item(i);
        if (node != null) {
            var matched = nodeMatchesDescription(node, descriptions[i]);
            if (matched)
                continue;
        }
        return false;
...
exports.nodeListMatchesDescription = nodeListMatchesDescription;
/**
    ...
    if (typeof first === 'object' && first != null) {
        if (typeof second === 'object' && second != null) {
            if (Array.isArray(first)) {
                if (Array.isArray(second)) {
                    for (var i = 0; i < first.length; i++) {
                        var matched = checkEquivalence(first[i], second[i]);
                        if (!matched)
                            return false;
                    }
            ...
            for (var key in first) {
                var matched = checkEquivalence(first[key], second[key]);
                if (!matched)
                    return false;
            }
 ...
 * @param {Record<string, string>} description - required values
 */
function applyDescribedAttributeChanges(attributes, description) {
    for (var key in description) {
        var attribute = attributes.getNamedItem(key);
    ...
    for (var i = 0; i < attributes.length; i++) {
        ...
        if (attribute == null || attribute.name in description)
            continue;
        attributes.removeNamedItem(attribute.name);
    }
}
exports.applyDescribedAttributeChanges = applyDescribedAttributeChanges;
/**
 ...
 */
function applyDescribedNodeChanges(node, description) {
    var _a;
    if (node.nodeName === description.nodeName) {
        if (node instanceof CharacterData) {
            var data = description.nodeValue;
            if (data != null && node.data !== data) {
                node.data = data;
            }
        }
        if (node instanceof Element) {
            var attributes = (_a = description.attributes) !== null && _a !== void 0 ? _a : {};
    ...
    var replacement = createDescribedNode(description);
    return replacement;
}
exports.applyDescribedNodeChanges = applyDescribedNodeChanges;
/**
 * Sets the child nodes of the target node to match the provided descriptions.
 * @function
 ...
 */
function setChildNodesFromDescriptions(node, descriptions) {
    ...
    for (var i = 0; i < descriptions.length; i++) {
        ...
        if (child != null) {
            var updatedChild = applyDescribedNodeChanges(child, description);
            if (updatedChild != null && child !== updatedChild) {
                node.replaceChild(updatedChild, child);
            }
        }
        else {
            var newChild = createDescribedNode(description);
            if (newChild != null)
                node.appendChild(newChild);
        }
    }
}
exports.setChildNodesFromDescriptions = setChildNodesFromDescriptions;
